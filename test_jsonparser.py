"""jsonparser 단위 테스트. 표준 unittest 만 사용."""
...
class TestGetPath(unittest.TestCase):
    ...
    def test_escaped_dot_key(self):
        self.assertEqual(get_path(DATA, r"weird\.key"), "dot")

    def test_get_all_always_list(self):
        self.assertEqual(get_all(DATA, "user.name"), ["Kim"])
        self.assertEqual(get_all(DATA, "no.such"), [])

    def test_string_not_indexed_as_sequence(self):
        # 문자열은 시퀀스지만 인덱스 경로 대상이 아님(키 접근만).
        self.assertEqual(get_all(DATA, "user.name[0]"), [])
